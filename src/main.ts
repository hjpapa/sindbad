...
const host: Host = {
    ...
    reward(r: Reward) { const before = progression(this.save.totalXp).level, beforeSkill=this.save.equippedSkill; this.save = grantReward(this.save, r); if(beforeSkill!==this.save.equippedSkill)hudCache=''; if (progression(this.save.totalXp).level > before) {
    ...
    dialogue(id: string, done: () => void) { stage.freeze(true); const d = dialogues[id]; let line = 0; const finish = () => { done(); resume(); }; const show = () => { panel(`<p class="eyebrow">항해의 대화</p><div class="dialogue"><img src="/assets/draft/${id === 'waveHint' || id === 'descent' || id === 'naira' || id === 'nairaHint' ? 'naira' : ['hazil','crystalHint','starMap','crystalJournal'].includes(id) ? (id==='starMap'?'starMap':id==='crystalJournal'?'journal':'genie') : id === 'rah' || id === 'flameHint' ? 'rah' : id === 'siren' || id === 'freed' ? 'siren' : 'player'}.svg" alt="${d.name} 간이 초상"><div><h2>${d.name}</h2><p data-testid="dialogue-text">${d.lines[line]}</p><small>${line + 1} / ${d.lines.length}</small></div></div><div class="buttons"><button id="next">${line === d.lines.length - 1 ? '대화 마치기' : '다음'}</button><button class="secondary" id="skip">전체 생략</button></div>`); el('next').onclick = () => { if (++line < d.lines.length)
        show();
    else
