...
- 자체 제작 SVG 34개와 12개 지역 팔레트 기반 Phaser 배경을 사용한다. 외부 이미지·음악은 없다. 기능용 벡터 아트는 전 구간에 있으나 명세의 최종 한국 웹툰 프레임/표정 애니메이션 수준에는 도달하지 않아 `ART_DRAFT`를 유지한다.
- 2026-09-23 최신 로컬 검사: typecheck 통과, lint 통과, 단위 57개 통과, 콘텐츠 검사 통과, production build 통과. `npm run test:e2e`의 17개 시나리오는 모두 `ok`로 완료됐다. 여기에는 기존 S01~S08 회귀, S28 평화 퍼즐/T06 저장, S36 엔딩, S09~S36 전 장면 렌더/콘솔 오류 검사가 포함된다. 다만 모든 결과 출력 뒤 Windows에서 Playwright 프로세스가 종료되지 않아 강제 종료했으며 명령 자체의 exit 0은 미확인이다.
- 앱 내 브라우저는 `privileged native pipe bridge is not available; browser-client is not trusted`로 연결할 수 없었다. 설치된 Edge headless는 실행 가능하다. 실제 휴대폰/태블릿, Safari/Firefox, 어린이 대상 난이도·3~7분 분량, 장시간 FPS/메모리, 최종 아트 승인, 배경 음악은 미검증이다.
