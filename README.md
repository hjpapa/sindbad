...
한국어 횡스크롤 액션 어드벤처의 첫 구현입니다. M0 + M1에 이어 M2 첫 구간까지 구현했으며 실제 맵은 S01~S05입니다. 전체 36개 스테이지는 설계 데이터로 등록되어 있으며 S06~S36은 개발 중으로 잠겨 있습니다. 최종 웹툰 아트가 아닌 `ART_DRAFT` 버전입니다.
...
- `src/content/maps.ts`: S01~S05의 실제 지형·적·장치·하트·안전 체크포인트.
