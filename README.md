...
한국어 **2.5D 횡스크롤 액션 어드벤처**입니다. 실제 3D 캐릭터·지형·조명과 크림/민트 UI를 사용합니다. 물리와 전투는 Phaser 3.90, 입체 화면은 Three.js 0.180.0으로 구현했습니다. 좌우 이동·점프 방식이며 자유 3D 이동/카메라 회전은 제공하지 않습니다.

이번 범위는 M0 + M1, 실제 맵은 S01~S03입니다. 전체 36개 스테이지는 설계 데이터로 등록되어 있으며 S04~S36은 개발 중으로 잠겨 있습니다. 2026-09-08 사용자 요청으로 아트 방향을 변경했으며 현재는 `ART_DRAFT / 3D 프리뷰`입니다. 기존 저장을 그대로 이어갈 수 있습니다.
